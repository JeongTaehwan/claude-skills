...
"스타일은 도구로 자동화하고 리뷰는 설계에 쓴다"와 "팀 합의에 여지를 둔다"를 한 파일에 같이 적어 둔 형태.

```toml
# pyproject.toml — PEP 8 을 기계 규칙으로 옮긴 pycodestyle 계열을 Ruff 로 돌린다
[tool.ruff]
line-length = 88        # PEP 8 의 79 가 아니다. 문서가 팀 합의에 여지를 두므로 위반이 아니다
target-version = "py311"

[tool.ruff.lint]
select = [
  "E", "W",  # pycodestyle — 레이아웃·공백 규칙 본체
  "I",       # isort — 표준 라이브러리 → 서드파티 → 로컬 그룹 분리
  "N",       # pep8-naming — lower_case 함수, CapWords 클래스, UPPER_CASE 상수
]

[tool.ruff.lint.isort]
known-first-party = ["myapp"]

# "어리석은 일관성" 절의 실행형 — 규칙이 오히려 읽기를 해치는 곳은 범위를 정해 연다
[tool.ruff.lint.per-file-ignores]
"migrations/*.py" = ["E501"]
```

이 설정이 덮는 건 PEP 8 중 기계로 판정 가능한 부분뿐이다. 이름이 실제로 무엇을 가리키는지, 주석이 맞는 말인지, 공개 API 경계를 밑줄 접두사로 제대로 그었는지는 여전히 사람이 본다 — 자동화의 목적은 그쪽에 리뷰 시간을 남기는 것이지 리뷰를 없애는 게 아니다.
