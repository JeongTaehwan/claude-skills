...
## 이럴 땐 아니다
- 위험의 지형을 훑고 우선순위를 잡는 단계라면 ASVS 는 너무 촘촘하다 — `development/owasp-top-10.md` 부터
- "그래서 JWT 를 어디에 저장하나" 같은 구현 방법의 답은 요구사항 표준이 아니라 `development/owasp-cheat-sheet-series.md`
- 설계 단계에서 이 시스템의 공격 표면을 발굴하는 일은 `development/owasp-threat-modeling.md`
- 코드 결함 유형의 분류 체계는 `development/cwe-top-25-most-dangerous-software-weaknesses.md`
- 조직 차원의 개발 프로세스 요구사항(빌드·릴리스·공급망 포함)은 `development/nist-secure-software-development-framework.md`
