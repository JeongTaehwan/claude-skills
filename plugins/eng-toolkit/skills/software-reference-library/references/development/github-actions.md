...
- 이미지 빌드 자체(레이어, 캐시, 크기)는 `infrastructure/docker.md`
...
문서의 보안 권고 세 가지 — 토큰 권한 최소화, 서드파티 액션 SHA 고정, 장기 키 대신 OIDC — 를 한 워크플로에 모아 둔 형태.
...
# 기본을 읽기로 좁히고, 필요한 것만 연다
permissions:
  contents: read
  id-token: write   # OIDC 단기 토큰 발급용

jobs:
  deploy:
    runs-on: ubuntu-latest
    environment: production   # 승인 게이트가 걸리는 지점
    steps:
      # 태그가 아니라 커밋 SHA 로 고정 (태그는 옮겨 달 수 있다)
      - uses: actions/checkout@<40자리-커밋-SHA>  # v4.2.2
      - uses: aws-actions/configure-aws-credentials@<40자리-커밋-SHA>  # v4
        with:
          role-to-assume: arn:aws:iam::123456789012:role/deploy
          aws-region: ap-northeast-2
```

SHA 는 각 액션의 릴리스 페이지에서 복사해 넣고, Dependabot 으로 갱신되게 두는 편이 현실적이다 — 고정만 해 두면 보안 패치가 영영 안 들어온다. 그리고 이 파일은 `pull_request_target` 을 쓰지 않는데, 포크 PR 에서 시크릿이 열리는 사고가 대개 그 트리거에서 시작된다.
