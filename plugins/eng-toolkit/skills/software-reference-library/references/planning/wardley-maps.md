...
doctrine 과 gameplay 를 한 문서에서 물리적으로 갈라 놓은 형태 — 섞여 있으면 "이번 상황의 수"가 슬그머니 팀 규칙으로 굳고, 그 반대도 일어난다.

```markdown
# 기술 전략 — 주문/결제 (2026)

## Doctrine — 상황과 무관하게 지킨다 (바꾸려면 이 문서를 고쳐야 한다)
- 사용자 니즈에서 시작한다. 모든 구성요소는 앵커에 연결되지 않으면 지도에 없다.
- 진화 단계가 다른 구성요소에 같은 개발 방식을 적용하지 않는다.
  - genesis: 실험, 실패 허용, SLA 요구 금지
  - custom: 팀이 직접, 반복 개선
  - product / commodity: 사온다. 직접 만들면 근거를 문서로 남긴다.
- 공통 어휘를 쓴다. 같은 것을 두 이름으로 부르지 않는다.

## Gameplay — 이번 판에서만 고른 수 (다음 분기에 뒤집혀도 정상)
- 결제 PG 를 한 곳에 몰아 협상력을 얻는다 (2026 상반기 한정)
- 검색은 매니지드로 미룬다 — 트래픽이 지금 규모면 자체 개발 회수 기간이 안 나온다
- 이탈 예측은 한 팀이 6주만 본다. 성과 없으면 접는다.

## 판정
새 규칙을 제안할 때 먼저 묻는다: 이건 언제나 지킬 것인가(doctrine),
아니면 지금 지형에서만 유리한 수인가(gameplay). 답을 못 하면 아직 둘 다 아니다.
```

두 목록으로 나눠도 gameplay 항목에 만료 조건이 없으면 결국 doctrine 처럼 굳는다 — 위 예시에서 유일하게 그걸 막는 것은 "2026 상반기 한정", "6주만" 같은 기한 표기다.
