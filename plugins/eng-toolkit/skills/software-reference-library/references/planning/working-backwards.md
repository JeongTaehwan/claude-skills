...
원문이 정한 네 문서의 순서를 그대로 파일 하나의 절 순서로 옮긴 것 — 화면·필드·API 가 등장하지 않는다는 점이 이 골격의 전부다.

```markdown
# [PR/FAQ] 결제 실패 재시도

## 1. Press Release (먼저 쓴다)
**오늘부터 카드 결제에 실패해도 장바구니를 다시 채울 필요가 없습니다.**
결제가 실패하면 무엇이 문제인지 한 문장으로 안내하고, 같은 화면에서 바로
다시 시도할 수 있습니다. 지금까지는 실패 코드만 보고 고객이 스스로
원인을 짐작해야 했습니다.
> 한 줄로 못 쓰는 기능은 이 릴리스에서 뺀다.

## 2. FAQ (고객이 물을 것)
- 왜 실패했는지 정확히 알려주나요? — 한도·유효기간·비밀번호·기타 4종으로 안내합니다.
- 카드를 바꿔야 하나요? — 아니요, 같은 카드로 다시 시도할 수 있습니다.
- 재시도하면 두 번 결제되나요? — 아니요. (내부 확인 필요: 중복 승인 방지 근거)

## 3. Customer Experience (고객이 실제로 겪는 흐름)
결제 실패 → 사유 문장 + [다시 시도] → 성공 시 기존 완료 화면과 동일
(목업 또는 유스케이스 서술. 여기서 처음으로 화면이 등장한다)

## 4. User Manual (설명해 보면 복잡도가 드러난다)
"결제가 실패하면 화면의 안내를 확인하고 다시 시도를 누르세요."
> 매뉴얼이 길어지면 설계가 복잡한 것이다.
```

이 문서를 잘 썼다는 것은 이야기가 설득력 있다는 뜻이지 수요가 있다는 뜻이 아니다 — 보도자료는 발견을 대체하지 않는다.
