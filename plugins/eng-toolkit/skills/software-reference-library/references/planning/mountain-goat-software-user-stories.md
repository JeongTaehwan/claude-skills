...
스토리 템플릿과 INVEST를 리파인먼트 체크리스트로 옮긴 형태 — 분할 축을 항목으로 박아 두면 "계층별로 자르자"는 기본값이 나오기 어려워진다.

```markdown
## [쿠폰] 장바구니에서 쿠폰 코드 적용

As a 재구매 고객
I want 장바구니에서 쿠폰 코드를 직접 입력해
so that 결제 단계까지 가기 전에 최종 금액을 확인할 수 있다

### 인수 조건
- [ ] 유효한 코드 입력 시 할인액과 최종 금액이 즉시 갱신된다
- [ ] 만료된 코드 입력 시 사유가 표시되고 금액은 변하지 않는다

### INVEST 점검
- Independent: 다른 스토리 완료를 기다리지 않는다 — 예
- Negotiable: 확정한 것은 위 인수 조건뿐, UI 배치는 미확정
- Valuable: 이 조각만 나가도 고객이 쓸 수 있다 — 예
- Estimable: 추정 불가면 스토리가 아니라 이해가 부족한 것
- Small: 한 스프린트 안 — 아니면 아래 축으로 세로 분할
- Testable: 인수 조건이 통과/실패로 갈리는가

### 분할 축 (수평 금지: "API 작업"/"화면 작업")
워크플로 단계 / 비즈니스 규칙 / 데이터 종류 / 인터페이스 / 성능 요건
```

INVEST를 통과했다고 스토리가 맞는 것은 아니다 — 형식 점검일 뿐, 그 스토리를 만들 가치가 있는가는 우선순위 쪽 판단이다.
