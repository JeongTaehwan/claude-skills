...
```bash
python3 scripts/token-usage.py --record --note "무엇을 바꿨는지"
...
남는다. **창(`--days`)을 반드시 함께 남긴다.** 최근 7일 수치와 전체 기간 수치는
비교 대상이 아니라서, 창을 모르면 나중에 둘을 나란히 놓고 틀린 결론을 낸다.

보는 축은 둘이다 — **응답 1회가 다시 읽는 컨텍스트**와 **바닥값**(시스템 프롬프트
+ 도구 정의 + 스킬 description + CLAUDE.md). 합계는 그 기간에 얼마나 일했는지에
좌우되므로 그것만 보고 좋아졌다/나빠졌다를 판단하지 않는다.

바닥값은 세션의 첫 응답이 아니라 **최솟값**으로 잡는다. `--days` 로 자르면 첫
응답이 이미 커져 있는 세션이 섞여서 첫 값이 바닥이 아니다.

**2026-08-23 기준선** (전체 기간, 세션 30개 · 응답 3,603개)

| | |
|---|---|
| 합계 | 1,216.9M (캐시읽기가 98.5%) |
| 응답 1회당 컨텍스트 | 중앙값 286,743 · 상위10% 685,794 · 최대 999,602 |
| 바닥값 | 29,346 × 응답 3,603개 = 105.7M (9%) |
| 턴당 증가 중앙값 | 989 |

한 턴이 더하는 양은 작다. **비용은 그게 이후 모든 턴에서 다시 읽히기 때문에**
세션 턴 수에 제곱으로 붙는다. 이 저장소가 지금까지 손댄 것(라이브러리 조회를
1/12로)은 턴당 증가를 줄인 것이지 턴 수를 줄인 게 아니다.
